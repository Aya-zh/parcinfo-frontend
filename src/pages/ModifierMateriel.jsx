...
export default function ModifierMateriel() {
  ...
  useEffect(() => {
    api.get(`/api/materiels/${id}`)
      .then(res => {
        const m = res.data;
        setForm({
          codeInventaire: m.codeInventaire || '',
          marque: m.marque || '',
          ...
          etat: m.etat || 'DISPONIBLE',
          dateAcquisition: m.dateAcquisition || '',
          dateGarantie: m.dateGarantie || '',
          valeur: m.valeur || '',
        });
      })
      .catch(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.put(`/api/materiels/${id}`, payload);
      navigate('/materiels');
    } catch (err) {
      const msg = err.response?.data?.message || err.response?.data || 'Erreur lors de la modification';
      alert(msg);
    } finally {
      setLoading(false);
    }
      ...
      <main className="db-main">
        <header className="db-topbar">
          <div className="db-search">
            <span style={{ color: '#64748b', fontSize: 14 }}>Modifier un matériel</span>
          </div>
