 ...
 const menuItems = [
  ...
  useEffect(() => {
    api.get('/api/pannes')
      .then(res => {
        let data = res.data;
        // Filtrer par bénéficiaire si le rôle est BENEFICIAIRE
        if (role === 'BENEFICIAIRE') {
          data = data.filter(p => String(p.beneficiaire?.id) === String(userId));
        }
        setPannes(data);
        setFiltered(data);
        setLoading(false);
        ...
        <nav className="db-nav">
          {menuItems.map((item) => (
            ...
            >
              ...
              {sidebarOpen && item.badge > 0 && <span className="db-badge">{item.badge}</span>}
            </div>
          ))}
        </nav>
        ...
        <div className="db-content">
          <div className="mat-header">
            <div>
              <h1 className="mat-title">
                {role === 'BENEFICIAIRE' ? 'Mes Pannes' : 'Gestion des Pannes'}
              </h1>
          ...
          <div className="mat-toolbar">
            <div className="mat-search">
              <FiSearch size={16} color="#64748b" />
              <input placeholder="Rechercher une panne..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          ...
          <div className="mat-filters">
            {['Tous', 'En attente', 'En cours', 'Résolu', 'Escaladée'].map(f => (
              <button
                ...
                style={{
                  border: activeFilter === f ? 'none' : f === 'En attente' ? '1px solid #F59E0B' : f === 'En cours' ? '1px solid #3B82F6' : f === 'Résolu' ? '1px solid #22C55E' : f === 'Escaladée' ? '1px solid #EF4444' : '1px solid #1A2B4A',
                  color: activeFilter === f ? '#fff' : f === 'En attente' ? '#F59E0B' : f === 'En cours' ? '#3B82F6' : f === 'Résolu' ? '#22C55E' : f === 'Escaladée' ? '#EF4444' : '#64748b',
              ...
              <table className="mat-table">
                <thead>
                  <tr>
                    <th>Description</th>
                    <th>Matériel</th>
                    {role !== 'BENEFICIAIRE' && <th>Bénéficiaire</th>}
                    <th>Gravité</th>
                    <th>Statut</th>
                ...
                <tbody>
                  {paginated.length === 0 ? (
                    <tr><td colSpan={8} style={{ textAlign: 'center', padding: '2rem', color: '#64748b' }}>Aucune panne trouvée</td></tr>
                  ) : paginated.map((p, i) => (
                    <tr key={p.id || i}>
                      ...
                      <td>
                        <div className="mat-actions">
                          <button className="mat-action-btn mat-action-view" onClick={() => navigate(`/pannes/${p.id}`)}>
                            <FiEye size={15} />
                          </button>
                          {(role === 'ADMINISTRATEUR' || role === 'TECHNICIEN') && (
                            <>
                              <button className="mat-action-btn mat-action-edit" onClick={() => navigate(`/pannes/modifier/${p.id}`)}>
                                <FiEdit2 size={15} />
                              </button>
                              <button className="mat-action-btn mat-action-delete" onClick={() => handleDelete(p.id)}>
                                <FiTrash2 size={15} />
                              </button>
                            </>
                          )}
